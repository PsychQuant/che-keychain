...
- Every value — dialog, clipboard, stdin, and both `set-pair` fields — goes through one normalization: surrounding whitespace and line breaks are dropped (a paste usually carries a trailing newline). A secret that legitimately starts or ends with whitespace is stored without it; nothing inside the value is touched (#6).

### Added

- `set --from-clipboard`: shows a confirmation dialog with the destination (no input field — the paste problem lived in the input field), then takes the value from the clipboard and, once it is stored and read back, removes it from this Mac's clipboard — only if the clipboard has not changed since the read (a clipboard manager or Universal Clipboard may keep a copy; on failure it is left as is). `set --stdin`: takes exactly one line from a pipe, stopping at the line break (LF or CR) without waiting for EOF; refused rather than guessed: a terminal (interactive paste is what bracketed-paste control sequences mangle), more than one line of content, invalid UTF-8, or more than 64 KiB without a line break. `--stdin` prints the destination on stderr instead of a dialog — the caller already holds the value, so it is meant for trusted automation. The two are mutually exclusive, and `--secure` / `--label` / `--explain` are refused with them. The value never enters argv or stdout (#6).
- Every store is verified: after writing, the item is looked up with the same scope as the pre-write check and its value read back by reference (keychain prompts disabled) and compared byte-for-byte. Empty or different → the just-written item is removed again; on a rotation the previous value is re-stored and itself read back, or the report says the slot is now empty. Unreadable (locked keychain) or ambiguous → the item is left in place, and on a rotation the report says an unverified value has replaced the previous one. Missing → reported as nothing stored. The exit code is non-zero in every mismatch case and the message states exactly what was done. An empty or whitespace-only value is refused in `save()` itself, so it covers all three `set` sources and `set-pair` (#6).
