...
- `unset` prints what it removed (`✓ removed N account(s) under S: a, b`) or `nothing to remove under S`, and exits non-zero when some match could not be removed. Wrappers that matched the old `✓ removed S/A` line, or relied on exit 0 after a partial sweep, must adapt.
...
- `set` / `set-pair` on an existing item: the behaviour now depends on what its decrypt ACL says (#5). Before, `save()` ran `SecItemDelete` (status discarded) then `SecItemAdd`, so an item created by another program surfaced as `errSecDuplicateItem` (-25299) with no explanation.
  - **Decrypt ACL trusts this binary and nothing else** → the item is deleted by reference and re-created with the new value and the requested ACL (`--daemon` = allow-all). An in-place `SecItemUpdate` was rejected in review: it keeps whatever ACL the item already has, including an owner entry another program may have pre-planted. The old value is read first and re-stored (prompt-on-read) if the add fails.
  ...
  - **Not a file-keychain item** (data-protection / iCloud keychain) → refused with a pointer to Keychain Access; che-keychain can neither inspect nor delete it.
...
- `--service` / `--account` validation now checks the string as typed (not a whitespace-trimmed copy, which let a value wrapped in newlines through) and also rejects DEL (0x7f): keychain names must not contain control characters, and these names are echoed into messages.
