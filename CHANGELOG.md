...
- `set --from-clipboard`: takes the value from the clipboard and, once the value is stored and read back, removes it from this Mac's clipboard — only if the clipboard still holds what was read (a clipboard manager or Universal Clipboard may keep a copy; on failure it is left as is). `set --stdin`: takes the first non-blank line from a pipe, stopping at the line break without waiting for EOF (a terminal is refused, since interactive paste is exactly what bracketed-paste control sequences mangle; more than 64 KiB without a line break is refused rather than truncated). The two are mutually exclusive, and `--secure` / `--label` / `--explain` are refused with them. Neither source shows the dialog, so the destination is printed on stderr before the store. All three sources trim surrounding whitespace and line breaks. The value never enters argv or stdout (#6).
- Every store is verified: after writing, the value is read back by the written item's reference (keychain prompts disabled) and compared byte-for-byte. Empty or different → the just-written item is removed again (a rotation gets its previous value re-stored) and `stored value mismatch` is reported with a non-zero exit; if the removal itself fails, the message says so. Unreadable (locked keychain) or ambiguous → the item is left in place and reported, since nothing proves it is bad. An empty or whitespace-only value is refused before anything is written. This covers all three `set` sources and `set-pair` (#6).
