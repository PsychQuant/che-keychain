...
| Copies of the value in this process | the dialog's field, the stdin buffer (wipe attempted on success and errors after allocation, best-effort), the clipboard string and the read-back copies are ordinary process memory and are not zeroed reliably; a rotation or `--replace` also holds the OLD value while it runs — the `--replace` backup and its pre-delete re-read are zeroed when the command finishes, other copies (a plain rotation's copy, read-backs) are not; the pasteboard is emptied only after a verified store and only if unchanged |
