...
| Copies of the value in this process | the dialog's field, the stdin buffer (wipe attempted on success and errors after allocation, best-effort), the clipboard string and the read-back copies are ordinary process memory and are not zeroed reliably; a rotation or `--replace` also holds copies of the OLD value while it runs. A wipe is attempted, best-effort, on the `--replace` backup, the preflight check's copy, the pre-delete re-read and a plain rotation's copy — best-effort because the value arrives bridged from a buffer the Security framework allocated, which may be released unwiped; read-backs of the old or new value are not wiped; the pasteboard is emptied only after a verified store and only if unchanged |
