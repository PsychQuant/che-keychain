...
| Binary calls `SecItemAdd` to write to `login.keychain-db`; an existing item is re-created (delete by reference + add, old value read back when possible, for the restore rule under `--replace` below) by default only if its ACL trusts this binary alone; without --replace, anything else (another trusted application, or an allow-all entry — including our own `--daemon` items) is refused before the dialog opens; the refusal offers `set --replace` (backed up until the new value is verified) and `unset` (discards the old value), both the user's decision | (only this binary holds the value in memory, briefly) |
