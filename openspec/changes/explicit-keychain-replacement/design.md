...
Non-goals: atomic transactions, universal daemon read access, automatic migration, replacement without a readable backup, preservation of item label/comment/dates, changes to set-pair input sources, attributing an item found at the destination to a particular write by this or any other process, or replacing an item whose value cannot be read with prompts disabled. The criterion is that observation, not who created the item: items created by `security add-generic-password` (partition `apple-tool:`) are the tested case, and any item whose value this binary cannot read with prompts disabled behaves the same. Observed for #9 on 2026-09-23: an identical binary at another path is admitted and `--replace` succeeds; a copy re-signed ad hoc under another identifier is refused at the backup read. A different Developer ID-signed release was not tested. Issue #7's acceptance was narrowed to this on 2026-09-22 (decision comment on #7, corrected by errata 2026-09-23). The remedy wording follows the later decision of 2026-09-24 (https://github.com/PsychQuant/che-keychain/issues/7#issuecomment-5798429935), which supersedes the errata's "no deletion advice": the refusal states the observed cause, says che-keychain holds no copy of the old value, gives the way to keep it first, and offers `che-keychain unset` only as the user's decision to discard it (the same remedy plain `set` gives; tested on both `security` shapes). A caller must not make that decision (CLAUDE.md rule 7). Prompting for the login-keychain password during the backup would break the prompts-disabled invariant and is out of scope.
...
A noninteractive stdin replacement MUST NOT widen plaintext access to an existing item; rotating with --daemon an item that already has a plaintext allow-all entry is allowed. The widening decision is taken from the access settings captured in the backup and reconfirmed by the pre-delete observation, because the first inspection happens before the backup and another writer can tighten the ACL in between; the early classification stays only as a cheap refusal. Eligibility asks whether an allow-all entry that grants the plaintext (decrypt, any or export-clear) exists, not whether the item classifies as `allowAll`: an ACL that carries such an entry alongside named applications is already readable by everything, so rotating it widens nothing. An export-wrapped-only allow-all entry does not count (round-6 J1). The classification carries that scope (`allowAll(.plaintext | .wrappedOnly | .promptGated)`, and the same on `foreign`; `.promptGated` is a plaintext allow-all entry with a nonzero prompt selector, not counted as open because the selector's effect is not verified — a stored selector reads back byte-swapped (observed), and `rebuiltAccess` writes it swapped back so the rehearsal reproduces it exactly), so the dialog, the refusal text, the consent binding and the guard read one fact (round-7 K1). All tests use uniquely named synthetic services, never production credentials. Tests cover default refusal, no-backup refusal with original data unchanged, explicit replacement and recovery of bytes plus ACL. Deterministic Security API failure injection is restricted to the existing DEBUG seam pattern.
...
- The consent `--replace` binds to (the access class the dialog showed) is compared with the first inspection and again with the observation immediately before the delete (#22, fixed in round 8 after being recorded here as a residual in round 4). A change after that last observation is not covered: it is an observation, not a lock.
