...
- The consent `--replace` binds to (the access class the dialog showed) is checked at the first inspection only. For a non-daemon replacement an ACL change between that check and the backup is not compared against the class the user saw; the backup and pre-delete equality checks still refuse any change after the backup. Accepted as a residual for this change (round-4 verify G7/H5) and filed as #22 rather than widened here.
