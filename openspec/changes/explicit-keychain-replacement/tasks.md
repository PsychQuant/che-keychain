...
- [x] 17.3 Full suite green (136/0), then `/idd-verify --pr 17` round 15 — ran 2026-09-24, all six reviewers completed: FAIL (0 HIGH, 2 MEDIUM): the old-value "wipes" zero only a fresh copy of a value bridged from the Security framework's buffer (observed by the logic reviewer and the DA, reproduced by the coordinator); findings became section 18.

## 18. Round 16 — round-15 verify findings

- [x] 18.1 (MEDIUM) Reproduced: `CFDataCreate` → `as! Data` → `resetBytes` leaves the CFData buffer holding the secret and zeroes only a new copy. The old-value `resetBytes` calls (the `--replace` backup and its pre-delete re-read, the preflight copy, the access-unreadable branch, a plain rotation's copy) are removed — they added one more copy of the secret and wiped nothing the keychain returned. `replacementBackup` documents why. `testZeroingAValueBridgedFromTheKeychainDoesNotReachItsBuffer` pins the observed behaviour (it fails, prompting a README revisit, if Foundation ever wipes in place).
- [x] 18.2 (MEDIUM) The README "Copies" row says exactly one buffer is wiped — the stdin buffer the value is assembled in — and that everything else, every copy of the old value included, is released unwiped. The CHANGELOG says the old-value wipes were removed and why. "are zeroed", "wipe is attempted", "is wiped on the way out" and "zeroed like the backup" are banned by the pinning test.
- [x] 18.3 (#15 item 5) The stdin wipe is now TESTED, not discharged by a stated limit: a DEBUG observer sees the buffer after the deferred wipe on a success and on `stdinTooLong` (thrown inside the read loop); mutation-checked (removing the wipe fails it). The round-14 comment on #15 gave a wrong reason (the stdin buffer is not bridged) and is corrected.
- [x] 18.4 (LOW) Code comments that claimed in-place zeroing are gone; #7 Current Status resynced.
- [x] 18.5 Full suite green (138/0), then `/idd-verify --pr 17` round 16 — ran 2026-09-24, all six completed: FAIL (1 HIGH from Codex — the widening guard ignored the ACL prompt selector; 4 MEDIUM on the stdin-wipe test's strength and the chunk rationale); findings became section 19.

## 19. Round 17 — round-16 verify findings

- [x] 19.1 (HIGH) An allow-all plaintext entry counts as open to every application only when its prompt selector is 0 (both `security -A` and `--daemon` items were observed to use 0; a password-requiring selector reads back as 256). New scope `AllowAllScope.promptGated`, used by `classify`, `hasAllowAllPlaintextEntry`, the dialog, the refusals and the success line. RED test first: the guard judged a password-gated fixture open (`hasAllowAllPlaintextEntry` true); that fixture was in practice refused later by the recovery rehearsal (`policyNotReproducible`), so the guard fix is defence in depth. The no-dialog `--daemon` replacement now refuses it at the guard.
- [x] 19.2 (MEDIUM ×3) The stdin wipe test now proves the wipe hits the buffer's own storage (the DEBUG observer reports whether the storage address survived the wipe), uses a 36-byte value so heap storage is exercised on success, and was mutation-checked against a live alias (fails). The buffer reserves room for the largest accepted value first; README says storage outgrown past that and the chunks are released unwiped.
- [x] 19.3 (MEDIUM, DA) The chunk rationale ("bridged, cannot be wiped") was wrong — `availableData` returns Swift-native Data. The comment and README now say only that the chunks are not wiped; the #15 correction is itself corrected.
- [x] 19.4 (LOW) The bridging pin test reads a real keychain item through `SecItemCopyMatching` instead of a CFData it built; #7's Tasks mirror (duplicated by an earlier sync) and #9, #11–#15 Current Status resynced.
- [x] 19.5 Full suite green (140/0), then `/idd-verify --pr 17` round 17 — ran 2026-09-24, all six completed: FAIL (0 HIGH, 6 MEDIUM): the promptGated wording claimed a confirmation prompt that an observed run did not show; a nonzero selector reads back byte-swapped so `--replace` cannot rebuild it while the refusal offered `--replace`; "ANY application can read" ignored the partition-ID gate; spec/design not amended; findings became section 20.

## 20. Round 18 — round-17 verify findings

- [x] 20.1 `promptGated` is described only as "an allow-all entry that carries a prompt selector — not counted as open"; what the selector requires is stated as unverified (fail-closed). No text says "only after a confirmation prompt" (tested).
- [x] 20.2 (superseded by 21.1: the swap is compensated and such items are rebuilt) A nonzero selector reads back byte-swapped (1 → 256, observed), so `--replace` refuses such items at the rehearsal (`policyNotReproducible`, now pinned in the test). The promptGated refusal says `--replace` refuses the item in any form and offers only discarding; `replaceBackupTerms` names "rebuild its access settings exactly" and prompt-selector items as a tested refusal case. `rebuiltAccess` is not changed: the swap is platform behaviour, and the refusal is fail-closed.
- [x] 20.3 Allow-all access is described "at the application-ACL layer" (dialog, refusal, owner label, success line), since #11 observed partition checks still apply; CLAUDE.md rule 6 no longer says readers "keep reading … silently" — the new item carries this binary's partition list.
- [x] 20.4 spec.md (requirement text, a prompt-selector scenario, the dialog requirement) and design.md (the class list) amended; README's --stdin --daemon row lists the prompt-selector exception; the InputSource comment no longer claims one storage block.
- [x] 20.5 Filed #23 (an allow-all ChangeACL entry is ignored by `classify`) as out of scope: replacement narrows access, only the description is wrong, and the rule must not reclassify this binary's own `--daemon` items.
- [x] 20.6 Full suite green (141/0), then `/idd-verify --pr 17` round 18 — ran 2026-09-24, all six completed: FAIL (0 HIGH, 5 MEDIUM). The Devil's Advocate showed the round-18 premise wrong: the selector swap is symmetric, so "cannot be rebuilt" was a choice not to compensate; the other MEDIUMs (foreignOwned still offering --replace, a doomed dialog) followed from that premise. Findings became section 21.

## 21. Round 19 — round-18 verify findings

- [x] 21.1 (MEDIUM, DA) `rebuiltAccess` writes each selector byte-swapped back (a stored selector reads back swapped; observed 1↔256, 0x11↔0x1100), so the rehearsal reproduces any nonzero selector exactly and a consented `--replace` of such an item succeeds. RED test first (the rehearsal refused with `policyNotReproducible`), then GREEN. If a system behaves differently, the rehearsal's record comparison refuses — never a silent change. This retires the doomed-dialog and "foreignOwned still offers --replace" findings, whose premise was the refusal.
- [x] 21.2 The promptGated refusal offers `set --replace` again, naming the widening a `--daemon` replacement causes (dialog says so, --stdin refuses); every "cannot be rebuilt exactly" claim is withdrawn (refusal, `replaceBackupTerms`, doc comment, design.md, CHANGELOG). The dead `daemonEffect = ""` branch is gone.
- [x] 21.3 (LOW) Help's `--stdin --daemon` exception lists the prompt-selector case; the test fixture and comments no longer say "password" and pin the fixture's class (`.allowAll(.promptGated)`, the unattributable path); CLAUDE.md rule 6's partition sentence cites the observed access records; sibling issue bodies resynced.
- [x] 21.4 Full suite green (141/0), then `/idd-verify --pr 17` round 19 — ran 2026-09-24, all six completed: FAIL (0 HIGH, 1 MEDIUM from the DA: the rehearsal ran only after the dialog, so an item it refuses — e.g. a partition mismatch — still had a secret collected first). Codex found no high- or medium-severity defect. Findings became section 22.
