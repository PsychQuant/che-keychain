...
- [x] 20.2 A nonzero selector reads back byte-swapped (1 → 256, observed), so `--replace` refuses such items at the rehearsal (`policyNotReproducible`, now pinned in the test). The promptGated refusal says `--replace` refuses the item in any form and offers only discarding; `replaceBackupTerms` names "rebuild its access settings exactly" and prompt-selector items as a tested refusal case. `rebuiltAccess` is not changed: the swap is platform behaviour, and the refusal is fail-closed.
...
- [ ] 21.4 Full suite green, then `/idd-verify --pr 17` round 19 (Codex on, all reviewers completing).
